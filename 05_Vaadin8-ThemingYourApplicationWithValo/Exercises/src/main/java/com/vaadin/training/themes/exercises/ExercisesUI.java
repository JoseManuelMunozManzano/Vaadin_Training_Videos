...
public class ExercisesUI extends UI {
	...
	private TextField createLimitTextField() {
		...
		limit.addValueChangeListener(event -> {
			...
			// TODO Set a style generator to the expensesGrid column with the id
			// "expenses". The style generator should return a warning style
			// name when the months' expenses are over the monthly expense limit
			expensesGrid.getColumn("expenses").setStyleGenerator(monthlyExpense -> {
				return monthlyExpense.getExpenses() > getMonthlyExpenseLimit() ? "v-grid-warning" : null;
			});
		});
		return limit;
	}
